...
const ExperienceCard = ({ title, role, period, icon: Icon, children, className = '' }) => {
    ...
    `}>
            <div className="flex justify-between items-start mb-6">
                <div className="border-2 border-black p-2 bg-muted-slate flex items-center justify-center w-12 h-12">
                    {Icon && <Icon className="w-6 h-6 stroke-[2]" />}
                </div>
                <span className="font-space font-bold text-xs border-2 border-black px-2 py-1 bg-white">
                    {period}
                </span>
            </div>
